...
还打算做一个监测站，方便查看表现。对优选域名的监测已经陆续开始了。

对于有 **ipv6** 需求的情况，例如校园网免流，需要确保优选 ipv6 地址或优选的域名支持 ipv6。Cloudflare Worker 本身支持双栈进出站，并且理论上和至少从在我这里的表现来看国内使用 Cloudflare ipv6 比 v4 表现好很多。
