...
通过 Cloudflare 边缘计算节点代理流量，并实现 v4/v6 转换。目前开源社区有一些项目成熟度已较高，同时教程比较详细。使用优选域名需支持 ipv6 并禁用 ipv4，使用优选 ipv6 不用禁用。这套方案比较灵活，但对小白不太友好。~~不过群里有我维护的公益项目~~
...
如果追求稳定、安全和隐私可以考虑自建，但一定要有 v4/v6 双栈。

**优点**：可控，表现好坏取决于厂商

**缺点**：省不了几个钱

很显然网上已经有许多保姆级教程了。开源千星脚本就可以
...
最后，不出意外免流这个系列应该是完结了。所有的方法就是这些，后续公益项目的维护都在tg和dc群里更新（人多热闹）。除此之外也欢迎关注这个博客，各位站长也欢迎交换友链♪(^∇^\*)